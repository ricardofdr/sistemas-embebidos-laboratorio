...
const byte EEPROM_ID = 0x99; // used to identify if there is valid data in EEPROM
//constants used to identify EEPROM addresses
const int ID_ADDR = 0; // the EEPROM address used to store the ID
const int BLINK_ADDR = 1; // the EEPROM address used to store the blink rate
...
void setup()
{
  byte id = EEPROM_read(ID_ADDR); // read the first byte from the EEPROM
  if( id == EEPROM_ID){
    // goes here if the id value read matches the value saved when writing eeprom
    // Pode enviar aqui esta mensagem pela USART: "Using data from EEPROM"
    unsigned char hiByte = EEPROM_read(BLINK_ADDR);
    unsigned char lowByte = EEPROM_read(BLINK_ADDR+1);
    blinkRate = word(hiByte, lowByte); // word function
  }
